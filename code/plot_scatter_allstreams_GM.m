 function [] = plot_scatter_allstreams(hemis)
...
str(1)= load(['All_R1_ventral_',hemis,'.mat']);
str(2)= load(['All_R1_dorsal_',hemis,'.mat']);
str(3)= load(['All_R1_lateral_',hemis,'.mat']);
str(4)= load(['All_R1_earlyvisual_',hemis,'.mat']);
...
for stream=1:3
    ...
    for roi=1:length(str(4).roi_list)
        ...
        h1=scatter([(10.^age)],[VAL], 40, [(age)], 'filled',  'MarkerFacecolor', streamcolor{4}(roi,:),'MarkerEdgecolor', [.5 .5 .5]); %colormap([streamcolor{stream}(roi,:); streamcolor{stream}(roi,:)]); % colorbar('eastoutside');
    ...
    for roi =1:length(str(stream).roi_list) %% running a linear mixed model per roi
        ...
        set(gcf, {'DefaultAxesXColor','DefaultAxesYColor'}, {'white' 'white'});grid on;
        h1=scatter([(10.^age)],[VAL], 40, [(age)], 'filled',  'MarkerFacecolor', streamcolor{stream}(roi,:),'MarkerEdgecolor', [.5 .5 .5]); %colormap([streamcolor{stream}(roi,:); streamcolor{stream}(roi,:)]); % colorbar('eastoutside');
...
for stream=1:3
    ...
    for roi=1:length(str(4).roi_list)
        scatter([roi],[slP1{4}(roi)], 150, [roi],   'MarkerFacecolor', streamcolor{4}(roi,:),'MarkerEdgecolor', streamcolor{4}(roi,:));
        errorbar([roi], slP1{4}(roi), slPSE1{4}(roi), 'color', streamcolor{4}(roi,:), 'Linewidth',3);
    end
    
    for roi=1:length(str(stream).roi_list)
        scatter([3+roi],[slP1{stream}(roi)], 150, [roi],   'MarkerFacecolor', streamcolor{stream}(roi,:),'MarkerEdgecolor', streamcolor{stream}(roi,:));
...
xticks([0 200 400])
for roi=1:length(str(4).roi_list)
    scatter([roi],[inC1{4}(roi)], 150, [roi], 'filled',  'MarkerFacecolor', streamcolor{4}(roi,:),'MarkerEdgecolor', streamcolor{4}(roi,:));
    errorbar([roi], inC1{4}(roi), inCSE1{4}(roi), 'color', streamcolor{4}(roi,:), 'Linewidth',3);
end
    
    for roi=1:length(str(stream).roi_list)
        scatter([3+roi],[inC1{stream}(roi)], 150, [roi], 'filled',  'MarkerFacecolor', streamcolor{stream}(roi,:),'MarkerEdgecolor', streamcolor{stream}(roi,:));
